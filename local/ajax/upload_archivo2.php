...
function productos_cargar_nuevos($conexion) {
  ...
  $sqlSelect="SELECT articulosceleste.*
  FROM articulosceleste
  LEFT JOIN articulos ON CONCAT('C-',articulosceleste.idCeleste) = articulos.idOnce
  ...
  if ($resultadoSelect->num_rows) {
    while ($filasNuevas=$resultadoSelect->fetch_assoc()) {
      $arr2=array();
      $arr2['idOnce']='C-'.$filasNuevas['idCeleste'];
      $arr2['descripcion']=addslashes($filasNuevas['descripcion']).'-'.addslashes($filasNuevas['descripcion_adicional']);
  ...
  foreach ($array_productos_nuevos as $clave => $valor) {
    $sqlUpdate="Insert into articulos(idOnce,descripcion,precioSinIva,precioConIva, ".
    " codigoBarra, proveedor) Values".
    "('".
    $valor['idOnce']."','".
    $valor['descripcion']."',".
    $valor['precioSinIva'].",".
    $valor['precioConIva'].",'".
    $valor['codigoBarra']."','Celeste y Blanca".
    "')";
...
function productos_actualizar_precios($conexion) {
  ...
  if ($resultado->num_rows) {
    ...
    while ($fila=$resultado->fetch_assoc()) {
      $arr2=array();
      $arr2['idOnce']='C-'.$fila['idCeleste'];
      $arr2['descripcion']=addslashes($fila['descripcion']).'-'.addslashes($fila['descripcion_adicional']);
...
//Cargamos el Archivo
move_uploaded_file($_FILES['archivo']['tmp_name'],'../datos/datosCeleste.txt');
if (strtolower($_FILES['archivo']['name'])!='celeste.csv') {
  echo "Error: el archivo no pertenece a Celeste y Blanca";
} else {
            //die('upload celeste!!!');
            ...
            $fileEntrada = "../datos/datosCeleste.txt";
            $fileSalida = "../datos/salidaCeleste.txt";
            ...
            $sql_0= "TRUNCATE TABLE articulosceleste";
            $db->query($sql_0);
            ...
            $sqlLoadData="LOAD DATA LOCAL INFILE '../datos/salidaCeleste.txt' IGNORE
            INTO TABLE articulosceleste FIELDS TERMINATED BY ';'
            LINES TERMINATED BY '\n'
            (idCeleste,@descripcion,@descripcion_adicional,@codigoBarra, @precioSinIva,@precioConIva)
            ...
            descripcion_adicional=REPLACE(@descripcion_adicional,\"'\",'')";
