...
if(isset($_REQUEST['action']) && !empty($_REQUEST['action'])){ //si viene algo en accion
  if($_REQUEST['action'] == 'addToCart' && !empty($_REQUEST['id'])){ // si en accion viene addToCart y tb viene un id
    ...
    // get product details
    $query = $db->query("SELECT * FROM articulos WHERE id = ".$productID);
    ...
    $orden="INSERT INTO orden (cliente_id, precio_total, creado, modificado) VALUES ('".$idCliente."', '".$cart->total()."', '".date("Y-m-d")."', '".date("Y-m-d")."')";
    //die($orden);
    $insertOrder = $db->query($orden);
